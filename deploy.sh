...
DEPLOY_CMD=(
  ...
  --ingress="all"
  --set-env-vars="^@^GCP_PROJECT=${GCP_PROJECT}@GCP_REGION=${GCP_REGION}@APP_NAME=${APP_NAME}@GEMINI_MODEL=${GEMINI_MODEL}@GEMINI_LIVE_MODEL=${GEMINI_LIVE_MODEL}@APP_ENV=${PROD_ENV:-production}@DEBUG=false@IAP_ALLOWED_DOMAINS=${IAP_ALLOWED_DOMAINS}@PROJECT_NUMBER=${PROJECT_NUMBER}@FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-cait-db}@BRAND_PROFILES_KIND=${BRAND_PROFILES_KIND:-itau_brand_profiles}"
  --project="${GCP_PROJECT}"
  --quiet
)
